...
async function updateTables() {
  try {
    ...
    //! Etat relay.
    const lastId = await db.gestionAir.findOne({
      attributes: [[Sequelize.fn('max', Sequelize.col('id')), 'maxid']],
      raw: true,
    }).then(id => id.maxid);

    const updateEtatRelay = db.gestionAir.update(
      { etatRelay: 0 },
      { where: { id: lastId } }
    );
    ...
    console.error('Une erreur est survenue lors de la mise à jour des tables :', error);
  }
}
